...
sub filename {
	my ($self) = shift;
	return $self->packagename() . ".dmg";
}


sub extract {
	my $self = shift @_;
	my $mountpoint = '/tmp/'.$self->packagename();
	$self->shell("hdiutil mount -mountpoint $mountpoint ".$self->download_path());
	$self->shell("cp -rp $mountpoint ".$self->packagename());
...
sub php_build_arch_pre {
	...
	my $pdflib_extension_tarball = $self->extras_dir() . "/pdflib-2.1.5.tgz";
